...
function Delete({ agricultureId, onDelete  }) {
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(`http://localhost:3001/Agriculture/${agricultureId}`);
      onDelete(); 
      handleClose(); 
    } catch (error) {
      console.error("Erreur lors de la suppression du l'agriculture", error);
